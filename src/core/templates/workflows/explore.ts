...
### 4. 转入规划
- 设计确认后，**必须**转入 openspec-plan 创建正式变更
- 提供清晰的需求摘要供规划使用
...
- **收敛到设计** → 经过上面的收敛阶段后，必须转入 openspec-plan
...
\`\`\`
.aiknowledge/codemap/
├── index.md                    # 顶层目录：模块列表 + 一句话描述 + 链接
├── connections.md              # 模块间链路关系图（依赖、数据流、调用链）
├── <module>/
│   ├── index.md                # 模块目录：子页面列表 + 链接
│   ├── overview.md             # 模块概览：职责、边界、对外接口
│   ├── internals.md            # 内部结构：核心类/函数、数据流
│   ├── dependencies.md         # 依赖关系：依赖谁、被谁依赖、交互方式
│   └── <sub-topic>.md          # 按需扩展：复杂子模块或特殊机制
└── ...
\`\`\`

模块目录下的文件按需创建，不要求全部存在。overview.md 是最低要求。
...
**探索过程中：**
- 当你理清了一个模块的架构脉络（组件关系、数据流、关键接口），记下来
- 不要中断探索去写文档——先完成思考
...
地图文档的原则：
- **地图而非百科**：每个文件描述"是什么"和"怎么连接"，不描述实现细节
...
当探索充分、需求已经澄清时，自然进入收敛阶段：

1. **方案对比** - 提出至少 2-3 个可行方案，用 ASCII 图表可视化，分析优缺点
2. **设计确认** - 与用户讨论收敛到最终方案，确保明确批准
3. **硬性门控** - 设计获得批准前，绝不编写任何代码
4. **转入规划** - 确认后必须转入 openspec-plan 创建正式变更
...
- **收敛到设计** → 经过收敛阶段后，必须转入 openspec-plan
- **导致产出物更新**："已用这些决定更新 design.md"
- **仅提供清晰度**：用户得到了他们需要的，继续前进
- **稍后继续**："我们可以随时继续这个话题"
