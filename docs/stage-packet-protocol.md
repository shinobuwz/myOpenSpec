...
覆盖范围：`plan-review`、`tdd`、`verify`、`review` 四个 gate stage。
...
| `gate_status` | object | `{plan_review: "pass"/"fail"/"pending", tdd: "pass"/"fail"/"pending", verify: "pass"/"fail"/"pending", review: "pass"/"fail"/"pending"}` |
| `stages` | object | 各 stage 的判定结果（见 5.3） |
...
`stages` 包含 4 个 stage 的判定结果，各 stage 只存判定结果，不复制 specs/design/tasks 等上下文：

| stage | 写入者 | 写入字段 |
|-------|--------|----------|
| `plan-review` | opsx-plan-review | `decision`, `findings`, `metrics`, `reviewed_at` |
| `tdd` | opsx-tdd | `total_tasks`, `completed_tasks`, `all_green`, `tasks[]`（每条含 task_id/mode/red_passed/green_passed/refactor_passed/coverage/completed_at） |
| `verify` | opsx-verify | `decision`, `findings`, `metrics`, `verified_at` |
| `review` | opsx-review | `decision`, `findings`, `metrics`, `reviewed_at` |

上下文信息（trace 矩阵、需求列表、task 列表）由 opsx-report 渲染时从产出物文件（specs/、design.md、tasks.md）实时读取。

### 5.4 Trace Overview 降级规则

- 产出物文件不存在时（如已删除），trace 矩阵板块显示"数据不可用"
- 禁止将缺失数据伪装为 `0/0`
