...
- **如"可实施"**：
  1. 写入门控状态：`yq -i '.gates.task-analyze = "'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"' openspec/changes/<name>/.openspec.yaml`
  2. 必须转入 **opsx-implement**。这不是建议，是强制要求。
- **如"需补充"**：不写入 gates。必须回到 **opsx-tasks** 修正 tasks.md。禁止绕过直接进入实施。
- 所有发现已记录在分析报告中
