...
> "Use Skill tool to invoke opsx-knowledge. Context: just archived change '<name>'. Extract reusable lessons from this change's implementation, fixes, and review findings. Reuse existing pitfall entries when possible, refresh `last_verified_at` for still-valid knowledge, and mark disproved entries as `superseded` or `deprecated` instead of silently overwriting them."

**9. Codemap 更新（强制）**

使用 Task tool（subagent_type: "general-purpose"）：
> "Use Skill tool to invoke opsx-codemap. Context: just archived change '<change-name>'. Create or update codemap for modules affected by this change. If existing codemap entries are inconsistent with code, mark them `stale` before refresh, then restore them to `active` with updated `last_verified_at` once verified. If codemap does not exist yet, create it from scratch — do NOT skip."
