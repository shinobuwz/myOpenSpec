...
- grouped change 场景下，archive 的默认归档单元是当前 subchange；父 group 只有在所有 subchange 都已处理完且用户明确要求时才应整体归档。
- 如果 resolved change root 是 `openspec/changes/<group>/subchanges/<subchange>`，归档目标必须是顶层 `openspec/changes/archive/YYYY-MM-DD-<group>-<subchange>/`。
...
**直接写入：**
- `openspec/changes/archive/YYYY-MM-DD-<archive-slug>/`（通过目录移动归档）
...
6. **执行归档**
   ...
   - 只有在用户明确要求归档父 group 且所有 subchange 已完成时，才允许把 `openspec/changes/<group>/` 移到 `openspec/changes/archive/YYYY-MM-DD-<group>/`

   默认情况下，grouped change 归档的是 resolved subchange root，而不是父 group 目录本身。
   ...
   **检查目标是否已存在：**
   ...
   ```bash
   mv <resolved-change-root> openspec/changes/archive/YYYY-MM-DD-<archive-slug>
   ```

   如果此次归档的是 subchange，移动完成后父 group 仍保留在 `openspec/changes/<group>/`，用于继续管理剩余 subchanges；不得在父 group 内新增任何 `archive/` 目录。
...
**变更：** <change-name>
**模式：** <schema-name>
**归档至：** openspec/changes/archive/YYYY-MM-DD-<archive-slug>/
**specs：** <N> 个文件（如存在）
...
启动一个通用 subagent：
> "调用 opsx-knowledge。上下文：刚刚归档了变更 '<logical-change-name>'，归档路径为 `openspec/changes/archive/YYYY-MM-DD-<archive-slug>/`。从本次变更的实现、修复和审查发现中提取可复用经验。尽量复用已有的 pitfall 条目，对仍然有效的知识刷新 `last_verified_at`，对已失效的条目标记 `superseded` 或 `deprecated`，而不是静默覆盖。"

**9. Codemap 更新（强制）**

启动一个通用 subagent：
> "调用 opsx-codemap。上下文：刚刚归档了变更 '<logical-change-name>'，归档路径为 `openspec/changes/archive/YYYY-MM-DD-<archive-slug>/`。为本次变更影响的模块创建或更新 codemap。若现有 codemap 条目与代码不一致，先将其标记为 `stale`，验证后恢复为 `active` 并更新 `last_verified_at`。若 codemap 尚不存在，从头创建——不可跳过。"
