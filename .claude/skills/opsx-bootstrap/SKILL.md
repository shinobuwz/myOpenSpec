...
| Skill | 触发词 | 用途 |
|-------|--------|------|
| opsx-explore | 探索、调查、脑暴、讨论方案 | 探索想法、澄清需求、方案对比与设计收敛 |
| opsx-knowledge | 经验、沉淀、知识库 | 独立沉淀 pitfalls；**归档后强制执行** |
| opsx-plan | 规划、计划、创建变更 | 创建 OpenSpec change 和 artifact |
| opsx-plan-review | plan skill 完成后 | 规划审查：spec↔plan 一致性 |
| opsx-task-analyze | tasks 生成后 | 任务分析：plan↔tasks 一致性 |
| opsx-tdd | 测试、TDD、红绿重构 | 测试先行开发 |
| opsx-implement | 实施、实现、开始编码 | 按 tasks.md 逐项实施 |
| opsx-verify | 验证、检查、质量 | 三维验证检查 |
| opsx-review | 审查、代码审查、review | 独立代码审查 |
| opsx-archive | 归档、上线、发布 | 归档 + knowledge + codemap + git |
| opsx-codemap | codemap、架构图、模块地图 | 独立维护 .aiknowledge/codemap/ 架构认知地图 |
| opsx-auto-drive | 自动驱动、auto、优化 | AI 自主驱动完整工作流 |
...
2. 执行 `bash .claude/opsx/bin/changes-status.sh` 获取当前变更列表（含阶段和进度）；若输出"无活动变更"则说明无活动变更
