...
**间接副作用（通过下游 skill）：**
- change 目录下的规划、任务、验证、审查产物
- 代码和测试文件
- `.aiknowledge/codemap/` 和 `.aiknowledge/pitfalls/`

**边界约束：**
- auto-drive 自身负责迭代编排和留档，不直接定义新的 gate 文件格式
- 代码、测试、gates 的写入由下游 workflow skill 负责
...
**Phase 3 - 规划**：使用 opsx-plan 创建变更（生成 proposal + design + specs），plan-review 通过后由 opsx-tasks 生成 tasks，task-analyze 通过后进入实施

**Phase 4 - 实施**：使用 opsx-implement 逐项实施

**Phase 5 - 验证**：使用 opsx-verify 三维验证
