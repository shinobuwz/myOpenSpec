...
- 如请求是复杂需求，先由 `opsx-slice` 创建父 change + subchanges，并写入执行拓扑；`active_subchange` 仅是可选运行态字段。
...
2. **创建或接续 change**
   ...
   如需新建单个小 change：
   ```bash
   opsx changes init <name> spec-driven
   ```
...
4. **创建规划产出物**
   ...
   读取 resolved change root 下 `.openspec.yaml` 中的 schema 定义、模板和输出路径，按依赖顺序生成：
   - `proposal.md`
   - `specs/<capability>/spec.md`
   - `design.md`
