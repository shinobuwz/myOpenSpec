...
3. **渲染 HTML**
   ...
   ### 板块 4：Conflict Board
   - 如有冲突（同一 stage 不同 reviewer 对同一问题的 severity 或存在性分歧）：
     - 展示冲突点
     - 涉及的 reviewer（agent_role）
     - 各自的 finding
     - arbiter 裁决（如有 arbiter StageResult）
   - 无冲突：显示 "No conflicts detected"
