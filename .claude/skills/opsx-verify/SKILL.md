...
6. **subagent 审查**
   ...
   使用 Agent tool 派遣 1 个 subagent，顺序执行四个维度的检查。subagent 收到 VerifyPacket JSON 作为输入。
   ...
   **消除双读规则**：subagent 从 core_payload 获取结构化事实（task_completion、task_traces、tdd_summary 等），只在需要验证具体代码证据时通过 `optional_refs.source_refs` 中 `kind: code` 的路径回读文件。禁止重读 tasks.md / specs / design 全文来获取 core_payload 中已存在的信息。
   ...
   ## 审查维度
   顺序执行以下四个维度，每个维度输出一个 StageResult JSON。

   ### 一、完整性检查（agent_role: verify-completeness）
   ...
   ### 二、正确性检查（agent_role: verify-correctness）
   如 core_payload.artifact_presence.specs 为 false，输出 decision: "skip"。
   否则从 core_payload.requirements 获取需求，通过 source_refs 中的 spec 路径回读场景，并通过 `kind: "code"` 的 source_refs 验证实现。

   ### 三、一致性检查（agent_role: verify-consistency）
   如 core_payload.artifact_presence.design 为 false，跳过设计遵循检查。
   否则通过 source_refs 中的 design 路径回读关键决策，并通过 `kind: "code"` 的 source_refs 验证代码遵循。

   ### 四、测试留档检查（agent_role: verify-test-report）
   从 core_payload.tdd_summary 获取 TDD task 数量。
   如为 0，输出 "No TDD tasks — test-report.md check skipped"。
   否则检查 core_payload.test_report_present，缺失则报 critical。

   ## 输出要求
   每个维度输出一个 StageResult JSON：
   {
     "version": 1,
     ...
     "agent_role": "verify-completeness|verify-correctness|verify-consistency|verify-test-report",
     "summary": "一句话总结",
     "decision": "pass|pass_with_warnings|fail|skip",
     "metrics": {"findings_total": N, "critical": N, "warning": N, "suggestion": N},
     "findings": [
       {"id": "F1", "severity": "critical|warning|suggestion", "dimension": "...", "message": "...", "trace_id": "R?", "evidence_ref": "path:line"}
     ]
   }
...
7. **汇总验证报告**
   ...
   主 agent 收集所有 StageResult JSON：

   **冲突检测**：如两个 reviewer 对同一问题给出不同 severity，触发 arbiter：
   - arbiter 只读取冲突相关的 evidence_refs（Lazy Hydration）
   - arbiter 输出 StageResult，agent_role 为 "arbiter"
   - arbiter 不重跑全量验证
...
- **如果验证通过**：
  ...
  2. 将 VerifyPacket + 所有 StageResult 写入 `openspec/changes/<name>/context/run-report-data.json`（追加式更新：不存在则创建，已存在且 JSON 合法则合并，JSON 损坏则中止并报错，见步骤 5）
