...
**处理 `[direct]` 标签前，必须先做以下质疑检查：**

- task 的验收标准或描述中是否包含”函数”、”返回”、”计算”、”判断”、”调用”、”处理”、”状态变更”？
  → 如果是，**拒绝执行**，将该 task 标签改为 `[test-first]` 并通知用户，然后按 test-first 执行
- task 是否涉及已有逻辑的修改（哪怕是很小的改动）？
  → 如果是，改为 `[characterization-first]`

只有以下情况才是真正的 `[direct]`：纯 JSON/YAML 配置、纯 TypeScript interface/type 定义（无逻辑）、纯 CSS/样式、脚手架文件初始化。

- 如果任务是 `[test-first]`：调用 openspec-tdd 执行红绿重构循环
- 如果任务是 `[characterization-first]`：先固化旧行为测试，再修改代码
- 如果任务是 `[direct]`（通过上方质疑检查）：直接执行，无需测试
