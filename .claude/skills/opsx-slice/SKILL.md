...
**默认写入：**
- `openspec/changes/<group>/index.md`
...
- slice 写 `audit-log.md` 记录切分决策（见退出契约）
...
5. 在父级 `index.md` 中记录总目标、依赖关系、串/并行链路和可选推荐焦点
...
- 输出切分报告
- 持久化父级 `index.md`、`.openspec.group.yaml` 和各 subchange `proposal.md`
- 追加 `audit-log.md` 记录切分决策，格式：
  ```
  ## slice | <ISO8601 时间戳> | <SPLIT_MULTI_CHANGE|KEEP_ONE_CHANGE|NEED_EXPLORE_FIRST>
  理由：<一句话说明为什么选择此结论>
  能力簇：<N> 个
  subchanges：<列出 subchange 名称，逗号分隔；KEEP 时写 change 名称；NEED_EXPLORE 时写”无”>
  ```
  - `SPLIT_MULTI_CHANGE`：audit-log 写入父 change 目录（`openspec/changes/<group>/audit-log.md`）
  - `KEEP_ONE_CHANGE`：audit-log 写入单个 change 目录（`openspec/changes/<name>/audit-log.md`）
  - `NEED_EXPLORE_FIRST`：不写 audit-log（未创建 change 结构）
- 明确指出”下一步应使用哪个 skill”
