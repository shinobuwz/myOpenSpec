...
- 用户输入的 `<name>` 可以是顶层 change、父 change 或 `group/subchange` 简写。
- 执行前必须先运行 `opsx changes resolve <name>`，得到真实 change root。
- 如果用户给的是父 change，则优先按 `active_subchange` 恢复；若为空，再退化到 `suggested_focus`、`recommended_order` 的首项或唯一 subchange。
- 后文所有路径均指 resolved change root。
...
1. **确定要恢复的 change**
   ...
   如果用户显式提供了 change 名称，先 resolve 再使用它。

   如果没有提供：
   - 运行 `opsx changes list` 查看活动 change
   - 如果没有活动 change：停止恢复流程；若需求尚未切分，提示先用 `opsx-slice`，否则改用 `opsx-plan` 创建新 change
   - 如果只有 1 个活动 change：自动选择它；如该 change 是 group，则继续按 group 路由规则解析
   - 如果有多个活动 change：向用户询问要恢复哪个 change，不要猜测

2. **读取最小恢复状态**

   读取 resolved change root 下的 `.openspec.yaml`，以及以下真实文件状态：
