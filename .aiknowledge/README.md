...
1. **Source references**：事实来源的稳定引用。默认使用 `change:<id>`、`commit:<sha>`、`audit-log:<path>`、`review-report:<path>` 等现有产物作为来源，不复制一份 raw source。
2. **LLM-maintained wiki**：可维护知识层，包括 `codemap/` 和 `pitfalls/`。这层可以更新、合并、废弃，但必须保留来源和审计记录。
3. **Schema / guardrails**：本文件、各级 `index.md`、skill 规则和 lint 自检，约束 agent 如何维护知识。
