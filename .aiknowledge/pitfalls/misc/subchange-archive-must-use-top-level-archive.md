...
```diff
+ 单个 change: openspec/changes/archive/YYYY-MM-DD-<change>/
+ subchange: openspec/changes/archive/YYYY-MM-DD-<group>-<subchange>/
+ 严禁创建 openspec/changes/<group>/subchanges/archive/
```

## 要点

grouped change 的默认 archive 单元是当前 resolved subchange root。只要源路径在 `subchanges/<subchange>/` 下，目标就必须落在顶层 `openspec/changes/archive/`，并用 `<group>-<subchange>` 生成稳定 slug。
