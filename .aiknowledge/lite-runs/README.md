...
`lite-runs/` 存放历史 `opsx-lite` 执行后的事实记录。新增快速工作项不再写入本目录，应写入 `openspec/fast/<id>/`。

lite-run 是旧小改动的轻量审计记录，不是正式 OpenSpec change，不能包含 proposal/design/spec/tasks/gates。当前新增工作流状态使用 fast item 和 `fast:<id>` 来源引用。
...
- 本目录历史保留；不要新增 lite-run 承接 workflow runtime 状态。
- lite-run 保持事实化、简洁。
