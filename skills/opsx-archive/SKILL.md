...
- Git merge-back、checkpoint 和分支清理按 `~/.opsx/common/git-lifecycle.md`，并优先调用 `opsx` CLI lifecycle 命令。
...
- 如果 `.openspec.yaml` 存在 `git.change_branch` 或 `git.branch_required: true`，移动归档目录前必须执行 `opsx git merge-back <change|fast> <id>`；失败则停止归档并记录阻塞原因。
