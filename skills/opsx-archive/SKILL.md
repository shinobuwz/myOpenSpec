...
归档已完成的 OpenSpec change。路径算法见 `references/archive-routing.md`，归档后的 knowledge/codemap worker 规则见 `references/follow-up-workers.md`。
...
读取 `.openspec.yaml`，校验 `gates.verify` 和 `gates.review` 字段均存在。任一缺失则拒绝执行并提示缺失关卡；此校验不可被用户确认绕过。
...
后续 workers 按 `opsx-subagent` worker contract 执行；主 agent 仍负责归档移动、父 group route 更新和最终摘要。

## 退出契约

输出归档摘要：变更名、schema、归档路径、spec 数量和任何警告。告知用户可选 git 操作，git 仅在用户明确要求时执行。
