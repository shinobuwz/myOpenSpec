...
按 `opsx-subagent` 的 worker contract 启动一个 knowledge worker。Codex 默认、Claude Code 兼容映射、controller boundary、写入边界和 fallback 均以 `opsx-subagent` 为准。主 agent 仍负责归档移动、父 group route 更新和最终归档摘要；knowledge worker 只能写被授权的 `.aiknowledge/` 条目，不能改写归档产物或 `.openspec.group.yaml`。
...
按 `opsx-subagent` 的 worker contract 启动一个 codemap worker。该 worker 只能写被授权的 `.aiknowledge/codemap/` 条目和必要的 `.aiknowledge/logs/YYYY-MM.md`，不能改写归档产物、父 group route 或 gates。
