...
1. 运行 `opsx changes init-group <group-name>` 创建父 change 容器
2. 为每个 subchange 运行 `opsx changes init-subchange <group-name> <subchange-name> spec-driven`
...
slice 为每个 subchange 初始化的正式 `proposal.md` 默认使用中文，除非用户明确要求英文。代码标识、capability 名称、文件路径、命令和配置键名保持原文。
