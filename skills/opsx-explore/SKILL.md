...
- 正式变更探索结束后，下一步必须是 `opsx-slice`；低风险小改动可建议转入 `opsx-lite`。禁止从 explore 直接跳转到 `opsx-plan`。
- 需要搜索源码前必须 codemap-first：先读 `.aiknowledge/codemap/index.md`；目标模块缺失或 stale 时先调用 `opsx-codemap`。
- 大范围只读探索按 `opsx-subagent` explorer contract 委托；主 agent 保持 controller，负责收敛判断和用户可见结论。
...
6. 结束时给出清晰下一步：`opsx-lite` 或 `opsx-slice`。
