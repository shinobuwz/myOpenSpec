...
- `pitfalls/` 是 LLM 维护层，可以更新、合并和废弃，但必须保留来源引用。
- 每次新增、更新、合并、废弃或 lint 修复后，必须向当前月度日志追加审计记录，例如 `.aiknowledge/logs/YYYY-MM.md`；`.aiknowledge/log.md` 仅作为日志索引。
- 合并正式索引过的条目时，保留被合并条目作为 `superseded` tombstone；只有未索引、未引用、从未正式消费的孤儿文件才允许删除。
...
4. **检查是否可归并 / 复核**
   ...
   - 如果两个正式索引过的条目需要合并：保留 canonical 条目，将被合并条目标记为 `superseded` 并填写 `superseded_by`；在 canonical 条目的 `merged_from` 中记录来源，禁止默认删除历史文件
   - 只有未索引、未引用、从未正式消费的孤儿文件，才允许合并后删除
...
5. **创建或更新条目**
   ...
   **标签**：[语言/平台标签]
   ...
   如果条目被废弃但没有替代项，必须设置 `status: deprecated` 并填写 `deprecated_reason`。如果条目被替代，必须设置 `status: superseded` 并填写 `superseded_by`。
...
9. **索引一致性校验**（每次进入 opsx-knowledge 时无条件执行，无论是否有新写入）
   ...
   - **文件数 > 索引行数**（孤儿条目）→ 对每个未索引的条目文件：
     ...
     2. 高度相似 → 合并内容到已有条目；仅当该文件未被索引、未被引用、从未正式消费时才删除，否则标记为 `superseded` tombstone
...
10. **完成前强制自检清单**（每次写入后必须逐项确认）
   ...
   [ ] 条目 frontmatter 已包含 status / last_verified_at / last_verified_by / verification_basis / source_refs
   [ ] 若执行 merge，正式条目已保留 tombstone 或 superseded_by，未静默删除历史
   [ ] 若执行 deprecate，已填写 deprecated_reason
   [ ] 当前月度日志 `.aiknowledge/logs/YYYY-MM.md` 已追加本次新增/更新/合并/废弃/lint 修复记录，`.aiknowledge/log.md` 已包含该 shard 链接
...
- 合并正式条目时保留 tombstone，除非确认它只是未索引、未引用、从未消费的孤儿文件
- 每次知识变更必须追加当前月度日志 `.aiknowledge/logs/YYYY-MM.md`，但日常使用不需要读取历史日志
