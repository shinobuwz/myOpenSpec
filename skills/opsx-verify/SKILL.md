...
在输出任何成功结论前，必须能指出本轮实际读取或执行的证据：

- 已读取的权威产物：`tasks.md`、`specs/**/*.md`、`design.md`、`test-report.md`
- 已检查的实现文件或 diff
- 已运行的验证命令及退出状态
- 如存在人工验收项，已记录其已验 / 待验状态

历史命令输出、主观判断、subagent 口头成功报告都不能单独作为完成依据。subagent 报告必须被主 agent 转化为可引用的 StageResult / audit-log 证据。

## Change Root 解析

- `<name>` 可以是单个 change、父 change 或 `group/subchange` 简写。
- 执行前先运行 `opsx changes resolve <name>` 获取真实 change root。
- 后文所有 `tasks.md`、`specs/`、`design.md`、`audit-log.md`、`.openspec.yaml` 路径均指 resolved change root。
...
1. **如果没有提供变更名称，提示选择**
   ...
   运行 `opsx changes` 获取可用变更（含阶段和进度），并让用户选择。
...
3. **subagent 审查**
   ...
   按 `opsx-subagent` 的 reviewer worker contract 派遣 1 个只读 reviewer subagent。Codex 默认、Claude Code 兼容映射、controller boundary、只读写入边界和 fallback 均以 `opsx-subagent` 为准；本 skill 只定义 verify 的 spec compliance、验证维度和 StageResult 输出。

   主 agent 负责读取当前轮 fresh evidence、汇总 reviewer StageResult、追加 `audit-log.md` 和写入 `.openspec.yaml` gates。reviewer 口头成功描述不能单独作为完成、通过或可归档依据。

   reviewer 直接读取以下文件进行审查：
   - `openspec/changes/<name>/tasks.md`
   - `openspec/changes/<name>/specs/**/*.md`（存在时）
   - `openspec/changes/<name>/design.md`（存在时）
   ...
   subagent 在同一轮完成 spec compliance 审查和验证维度检查，输出 1 个 StageResult JSON（见 `docs/stage-packet-protocol.md` 第 1 节）。
   ...
   ## 审查维度
   顺序执行以下维度，并将所有 findings 合并到同一个 StageResult JSON 中。

   ### 零、Spec Compliance Review（dimension: VERIFY_SPEC_COMPLIANCE）
   对照 proposal.md、specs/**/*.md、design.md、tasks.md 和代码文件，确认实现满足当前 OpenSpec change。
   检查：
   - 每个需求是否有实现证据
   ...
   test-report.md 存在时，检查每个 `### ♻️ 重构阶段` 是否包含覆盖率字段。
   - 缺失覆盖率字段：报 critical，归类为 test-report 不完整。
   - 覆盖率为裸 `N/A`（例如 `**覆盖率**：N/A`，没有括号说明）：报 critical，归类为 test-report 不完整；要求补充不可用原因和补救命令/不适用说明。
   - 覆盖率为数字但没有采集命令或产物引用：报 suggestion，要求补充 evidence。

   ### 五、人工验证项检查（dimension: VERIFY_MANUAL）
   扫描 tasks.md 中所有 `[manual]` 验收标准。
   如为 0，在 summary 中注明 "No manual ACs"。
   否则：
   - 检查 test-report.md 中是否有对应的「⏳ 待人工验证」清单，缺失则报 warning
   - 统计 `[manual]` 项中已勾选 `[x]` 和未勾选 `[ ]` 的数量
   - 未勾选的 `[manual]` 项报 warning（非 critical），附提示：「需人工验证后方可确认完整性」
   ...
   输出一个 StageResult JSON：
   {
     ...
     "findings": [
       {"id": "F1", "severity": "critical|warning|suggestion", "dimension": "VERIFY_SPEC_COMPLIANCE|VERIFY_COMPLETENESS|VERIFY_CORRECTNESS|VERIFY_CONSISTENCY|VERIFY_TEST_REPORT", "message": "...", "trace_id": "R?", "evidence_ref": "path:line"}
     ]
   }
